...
invisible(gc())
doubleidx <- which(sapply(varcluster,length)==2)
varcluster[doubleidx]
length(doubleidx)

for(cluster_i in doubleidx){
  ...
  if(length(setdiff(clusterset,ignoreset_history))==0){
    ...
    rm(list=setdiff(ls(),c("varcluster","num_wtid","ignoreset_history","doubleidx","clusterset","cluster_i")))
    invisible(gc())
