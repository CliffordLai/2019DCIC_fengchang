...
# number of var, estimated time
# 1


# All singular cluster
rm(list=setdiff(ls(),c("varcluster","num_wtid","ignoreset_history")))
invisible(gc())
singleidx <- which(sapply(varcluster,length)==1)
unlist(varcluster[singleidx])
length(singleidx)

for(cluster_i in singleidx[11:15]){
  clusterset <- varcluster[[cluster_i]]
  cat("********************************\n Start Cluster of",clusterset,"\n\n")
  
  if(length(setdiff(clusterset,ignoreset_history))==0){
    cat("Ignore because of high score!")
    next
  }else{
    rm(list=setdiff(ls(),c("varcluster","num_wtid","ignoreset_history","singleidx","clusterset","cluster_i")))
    invisible(gc())
